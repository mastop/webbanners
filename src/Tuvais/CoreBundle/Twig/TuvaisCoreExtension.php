...
namespace Tuvais\CoreBundle\Twig;
 ...
 */
class TuvaisCoreExtension extends Twig_Extension {
    ...
    public function getFunctions() {
        $mappings = array(
            'tuvais_select_city' => 'selectCity',
            'tuvais_get_cities' => 'getCities',
        );
    ...
    public function selectCity($name = 'city', $id = null, $class = null, $style = null) {
        $repo = $this->container->get('mastop')->getDocumentManager()->getRepository('TuvaisCoreBundle:City');
        $cities = $repo->findAll();
        $ret = null;
        if ($cities) {
            $current = $this->container->get('session')->get('tuvais.user.city');
