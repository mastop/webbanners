<?php
namespace Tuvais\CoreBundle\Util;
...
class IPtoCity {
    ...
    function __construct(ContainerInterface $container, $ip) {
        ...
        if($container->hasParameter('tuvais.quova.apikey') && $container->hasParameter('tuvais.quova.secret')){
            $apikey = $container->getParameter('tuvais.quova.apikey');
            $secret = $container->getParameter('tuvais.quova.secret');
            $ch = curl_init();
            $ver = 'v1/';
            $method = 'ipinfo/';
