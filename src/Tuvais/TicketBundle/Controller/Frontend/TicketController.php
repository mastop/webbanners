...
namespace Tuvais\TicketBundle\Controller\Frontend;
...
use Tuvais\UserBundle\Form\Frontend\UserForm;
use Tuvais\UserBundle\Form\Frontend\UserFormTwitter;
use Tuvais\UserBundle\Form\Frontend\UserFormEdit;
use Tuvais\UserBundle\Form\ForgetForm;
use Tuvais\UserBundle\Form\Frontend\ReenviarForm;
use Tuvais\UserBundle\Form\Frontend\ChangePassForm;
use Tuvais\UserBundle\Form\Frontend\BankDataType;
use Tuvais\UserBundle\Document\User;
use Tuvais\UserBundle\Document\BankData;
...
class TicketController extends BaseController {
    ...
    public function checkCPFAction() {
        if ($this->get('request')->isXmlHttpRequest()) {
            if ($this->get('request')->getMethod() == 'POST') {
                ...
                if (!empty($cpf)) {
                    $result = $this->dm()->createQueryBuilder('TuvaisUserBundle:user')
    ...
    public function newAction() {
        ...
        return $this->render('TuvaisUserBundle:Frontend/User:novo.html.twig', array(
    ...
    public function editAction() {
        ...
        return $this->render('TuvaisUserBundle:Frontend/User:editar.html.twig', array(
                    'form' => $form->createView(), 'titulo' => $titulo,
                    'usuario' => $userLogado,
                ));
    }
    ...
    public function activeAction($actkey) {
        $repository = $this->mongo('TuvaisUserBundle:User');
    ...
    public function resendAction() {
        ...
        return $this->render('TuvaisUserBundle:Frontend/User:reenviar.html.twig', array(
    ...
    public function resendOkAction() {
        ...
        $repository = $this->dm()->getRepository('TuvaisUserBundle:User');
    ...
    public function detailsAction($username) {
        ...
        if (count($itens) > 0) {
            if ($this->verificaStatus($itens)) {
                return $this->render('TuvaisUserBundle:Frontend/User:detalhes.html.twig', array(
                            'usuario' => $itens));
            } else {
                return $this->redirect($this->generateUrl('_home'));
            }
    ...
    public function saveAction($id=null) {
        ...
        $factory = $this->get('form.factory');
        $cityId = $this->get('session')->get('tuvais.user.cityId');
        if ($id) {
        ...
        if ($form->isValid()) {
            if ($id) {
                $user = $this->dm()->getReference('TuvaisUserBundle:User', $dadosPost['id']);
    ...
    public function recoveryAction($username=null) {
        ...
        return $this->render('TuvaisUserBundle:Frontend/Security:forget.html.twig', array(
                    'form' => $form->createView(),
                ));
    }
    ...
    public function recoveringAction($actkey) {
        ...
        if (count($usuario) == 1) {
            ...
            return $this->render('TuvaisUserBundle:Frontend/User:changepass.html.twig', array(
        ...
        if ($this->get('request')->isXmlHttpRequest()) {
            ...
            if ($usuario || $usuario2) {
                ...
                if ($usuario) {
                    ...
                    $user = $this->dm()->getReference('TuvaisUserBundle:User', $usuario->getId());
                    ...
                    if (!isset($fbID) || $fbID == $gets->get('facebookId')) {
                        $user = $this->dm()->getReference('TuvaisUserBundle:User', $usuario2->getId());
                        $user->setName($gets->get('firstName') . " " . $gets->get('lastName'));
                        $city = $this->mongo('TuvaisCoreBundle:City')->findOneById($cityId);
                        $user->setCity($city);
                        if (!isset($fbID)) {
    ...
    function twitterBackAction() {
        ...
        if (is_object($dados)) {
            ...
            if ($usuario) {
                ...
                return $this->render('TuvaisUserBundle:Frontend/User:novoTwitter.html.twig', array(
