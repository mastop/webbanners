...
use Banner\UserBundle\Form\Frontend\ReenviarForm;
use Banner\UserBundle\Form\Frontend\ChangePassForm;
use Banner\UserBundle\Form\Frontend\BankDataType;
use Banner\UserBundle\Document\User;
use Banner\UserBundle\Document\BankData;
...
class UserController extends BaseController {
    ...
    public function newAction() {
        ...
        return $this->render('BannerUserBundle:Frontend/User:novo.html.twig', array(
    ...
    public function editAction() {
        ...
        return $this->render('BannerUserBundle:Frontend/User:editar.html.twig', array(
                    'form' => $form->createView(), 'titulo' => $titulo,
                    'usuario' => $userLogado,
                ));
    }
    ...
    public function resendAction() {
        ...
        return $this->render('BannerUserBundle:Frontend/User:reenviar.html.twig', array(
    ...
    public function saveAction($id=null) {
        ...
        if ($form->isValid()) {
            if ($id) {
                ...
                $father = $this->get('request')->cookies->get('bannerU');
                if($father){
                    $father = $this->mongo('BannerUserBundle:User')->findByCode($father);
                    if($father){
    ...
    public function recoveryAction($username=null) {
        ...
        return $this->render('BannerUserBundle:Frontend/Security:forget.html.twig', array(
                    'form' => $form->createView(),
                ));
    }
    ...
    public function recoveringAction($actkey) {
        ...
        if (count($usuario) == 1) {
            ...
            return $this->render('BannerUserBundle:Frontend/User:changepass.html.twig', array(
        ...
        if ($this->get('request')->isXmlHttpRequest()) {
            ...
            if ($usuario || $usuario2) {
                ...
                if ($usuario) {
                    ...
                    $user = $this->dm()->getReference('BannerUserBundle:User', $usuario->getId());
                    ...
                    if (!isset($fbID) || $fbID == $gets->get('facebookId')) {
                        $user = $this->dm()->getReference('BannerUserBundle:User', $usuario2->getId());
                        $user->setName($gets->get('firstName') . " " . $gets->get('lastName'));
                        if (!isset($fbID)) {
    ...
    function twitterBackAction() {
        ...
        if (is_object($dados)) {
            ...
            if ($usuario) {
                ...
                return $this->render('BannerUserBundle:Frontend/User:novoTwitter.html.twig', array(
