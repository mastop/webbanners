...
namespace Banner\UserBundle\Controller\Backend;
...
use Banner\UserBundle\Form\Backend\UserForm;
use Banner\UserBundle\Form\Backend\UserFormEdit;
use Banner\UserBundle\Form\Backend\ChangePass;
use Banner\UserBundle\Form\ForgetForm;
...
class UserController extends BaseController {
    ...
    public function indexAction() {
        ...
        $dados = $rep->getRepository("BannerUserBundle:User")->findAllByCreated();
        $itens = array();
        $titulo = $this->trans("Listagem de usuários");

        return $this->render('BannerUserBundle:Backend/User:index.html.twig', array(
                    'title' => $titulo,
                    'usuarios' => $dados));
    }
    ...
    public function newAction($username = false) {
        if ($username) {
            ...
            $rep = $this->mongo('BannerUserBundle:User');
            ...
            return $this->render('BannerUserBundle:Backend/User:novo.html.twig', array(
    ...
    public function passAction($username) {
        ...
        return $this->render('BannerUserBundle:Backend/User:senha.html.twig', array(
    ...
    public function savePassAction($id=null) {
        ...
        $repository = $this->mongo('BannerUserBundle:User');
        ...
        // /validando se a senha confere com a repetição
        if (count($erro) == 0) {
            $user = $this->dm()->getReference('BannerUserBundle:User', $dadosPost['id']);
    ...
    public function saveAction($id=null) {
        ...
        if ($form->isValid()) {
            if ($id) {
                ...
                $result = $this->dm()->createQueryBuilder('BannerUserBundle:User')
    ...
    public function deleteAction() {
        ...
        if ('POST' == $request->getMethod()) {
            $id = $request->get('id');
            $usuario = $this->mongo('BannerUserBundle:User')->findOneById($id);
            ...
            $this->mongo('BannerUserBundle:User')->block($usuario);
            ...
            $usuario = $this->mongo('BannerUserBundle:User')->findOneBy(array('username' => $username));
    ...
    public function infoAction(User $user){
        ...
        $ofertas = $this->mongo('BannerDealBundle:Deal')->count('user.id', $user->getId());
        $compras = $this->mongo('BannerOrderBundle:Order')->count('user.id', $user->getId());
        $vendas = $this->mongo('BannerOrderBundle:Order')->count('seller.id', $user->getId());
        $saldo = $this->mongo('BannerOrderBundle:Escrow')->totalCheckoutByUser($user->getId());
        return array(
            'title' => $title,
            'user' => $user,
            'ofertas' => $ofertas,
