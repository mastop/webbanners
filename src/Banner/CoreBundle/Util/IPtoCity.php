<?php
namespace Banner\CoreBundle\Util;
...
class IPtoCity {
    ...
    function __construct(ContainerInterface $container, $ip) {
        ...
        if($container->hasParameter('banner.quova.apikey') && $container->hasParameter('banner.quova.secret')){
            $apikey = $container->getParameter('banner.quova.apikey');
            $secret = $container->getParameter('banner.quova.secret');
            $ch = curl_init();
            $ver = 'v1/';
            $method = 'ipinfo/';
