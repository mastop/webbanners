...
class LoadParametersData implements FixtureInterface, ContainerAwareInterface {
    ...
    public function load(ObjectManager $manager) {
        ...
        $child->setValue('© 2012 <strong>Mastop</strong> - Internet Development | Todos os direitos reservados.');
        $param->addChildren($child);
        ...
        $child->setFieldtype('textarea');
        $child->setValue('webbanner, banner, banners, banner online, banners online');
        $param->addChildren($child);
        $child = new Children();
        $child->setName('description');
        $child->setTitle('Meta Description');
        $child->setDesc('Digite a descrição para a home do site');
        $child->setValue('A Webbanner faz banners personalizados online.');
        $child->setFieldtype('textarea');
        $param->addChildren($child);
        $manager->persist($param);
        $manager->flush();
